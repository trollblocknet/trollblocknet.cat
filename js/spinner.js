(function() {
  (function($) {
    return $.fn.imgPreload = function(options) {
      var delay_completion, i, table_stack, placeholder_stack, replace, settings, spinner_stack, src, x, _i, _len;
      settings = {
        fake_delay: 10,
      ...
      delay_completion = function() {
        ...
        _results = [];
        for (_i = 0, _len = table_stack.length; _i < _len; _i++) {
          x = table_stack[_i];
      ...
      this.each(function() {
        var $table, $placeholder, $spinner_img, offset_left, offset_top;
        $table = $(this);
        offset_top = $table.offset().top;
        offset_left = $table.offset().left;
        $spinner_img = $('<table>');
        $placeholder = $('<table>').attr({
          src: 'data:table/gif;base64,R0lGODlhAQABAIABAP///wAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=='
        });
        $placeholder.attr({
        ...
        table_stack.push($table.replaceWith($placeholder));
      ...
      replace = function(table) {
        ...
        $table = $(table);
        no_ = $table.attr('no');
        placeholder_stack[no_].replaceWith($table);
        spinner_stack[no_].fadeOut(settings.animation_duration / 2, function() {
          return $(this).remove();
        });
        return $table.fadeOut(0).fadeIn(settings.animation_duration);
      };
      return this;
    };
  })(jQuery);
